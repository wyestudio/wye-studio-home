-- p44: 인스타 이벤트 쿠폰 자동 발급 (외부 도구/ManyChat 연동)
--
-- ⚠️ 번호가 p44 인 이유: 처음 p41 로 만들었는데 같은 날 다른 작업의
--    p41_public_site_settings 와 겹쳤다(저쪽이 09:59, 이쪽이 11:29).
--    오늘만 네 번째 충돌이다(p31·p34·p38·p41).
--    CLAUDE.md 「마이그레이션 파일 이름 규칙」 대로 앞자리는 date 로 뽑고,
--    p 번호는 `ls supabase/migrations/` 로 확인한 뒤 정할 것.
--
-- 왜
