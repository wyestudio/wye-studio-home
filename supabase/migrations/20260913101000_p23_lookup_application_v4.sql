-- 참여내역 조회 v4 — 취소일·입금확인일 추가
--
-- 적용: test · 운영 (2026-09-13)
-- 되돌리기: drop function if exists public.lookup_application_v4(text, text);
--           (v3 는 그대로 두므로 앱만 되돌리면 된다)
--
-- v3 에 없어서 화면이 못 보여주던 것:
--   · cancelled_at — 취소한 신청의 '취소일'. 환불 금액을 계산할 때도 이 시점을
--                    기준으로 삼아야 한다(지금 시각으로 재면 행사가 지난 뒤에
--                    조회했을 때 환불 0원으로 보인다).
--   · paid_at      — '입금확인일'. v3 는 payment_confirmed_sms_sent_at 을 줬는데
--                    그 값은 어드민 수동 등록에서 문자 없이도 찍혀 의미가 흐리다.
--                    (p20 에서 의미가 분명한 paid_at 으로 분리했다)
--
-- ⚠️ v3 를 고치지 않고 새 함수를 만든다. 반환 컬럼이 바뀌는 함수를 replace 하면
--    "cannot change return type of existing function" 으로 막히고, 억지로 drop
--    해서 바꾸면 배포 순서에 따라 옛 화면이 없는 컬럼을 읽어 터진다.
--    v2 → v3 때와 같은 방식이다.

create or replace function public.lookup_application_v4(
  p_phone_digits text,
...
comment on function public.lookup_application_v4(text, text) is
  '전화번호 + 접수번호로 참여내역 조회. v3 에 입금확인일(paid_at)·취소일·환불완료일을 더했다.';

revoke execute on function public.lookup_application_v4(text, text) from public;
grant execute on function public.lookup_application_v4(text, text) to anon, authenticated, service_role;
