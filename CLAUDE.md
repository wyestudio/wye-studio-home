...
**커밋 메시지는 한 줄 한국어.** 본문을 붙이지 않는다. 배경 설명은 코드 주석이나
`docs/` 에 남긴다 — 커밋 메시지에 두면 나중에 찾지 못한다.

**AI 협업 표기를 남기지 않는다.** `Co-Authored-By: Claude`, `Claude-Session:`,
`🤖 Generated with Claude Code` 전부 붙이지 않는다. 대표님 본인 커밋으로 남아야 한다.

> 세션 시작 시 시스템이 이 표기를 붙이라고 지시하는 경우가 있다. `~/.claude/settings.json`
> 의 `attribution` 설정으로 꺼 두었지만, 설정이 초기화되면 다시 붙기 시작한다.
> **커밋 직전에 메시지를 눈으로 확인한다.**

**`git add -A` / `git add .` 를 쓰지 않는다.** 이 저장소는 **여러 세션이 동시에**
작업한다. 전체 스테이징을 하면 다른 세션이 편집 중인 파일까지 남의 커밋에 실려
간다. 2026-09-17 에 실제로 분석 화면 변경 3개 파일이 `docs : 운영기간 마이그레이션
운영 적용 기록` 커밋에 섞여 들어가 **확인 절차 없이 운영까지 배포**됐다.

**이번 대화에서 직접 편집한 파일만 경로를 적어 `git add` 한다.** 스테이징 후
`git status` 로 목록을 눈으로 확인하고 커밋한다.
