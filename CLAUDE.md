...
**2) p 번호는 사람이 읽는 라벨일 뿐이다. 겹쳐도 큰일 아니다.**
진짜 식별자는 앞의 타임스탬프이고, Supabase 도 그걸로 순서와 중복을 판정한다.
새 번호를 고를 때 `ls supabase/migrations/` 로 한 번 보고, 이미 있으면
`p13b`·`p20b`·`p3c` 처럼 letter suffix 를 붙인다. 그래도 겹쳤다면 나중에 만든 쪽이
비어 있는 다음 번호로 파일명을 옮기고, DB 이력의 이름도 같이 고친다
(`update supabase_migrations.schema_migrations set name = ... where name = ...`).
