...
> 📌 **별건 확인 요청**: 취소·결제취소 상태인데 `refund_completed_at`이 비어 있는 건이 **16건** 있다. `refund_completed_at` 컬럼이 2026-08-27에 추가됐으므로 그 이전 환불이 표시만 안 된 것일 수 있으나, **실제 미환불인지 확인이 필요**하다. 이번 작업과 무관하지만 고객 돈이 걸린 사안이라 남겨둔다.
