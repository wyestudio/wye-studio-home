...
## 유입 추적(UTM) — 2026-09-17 시점

2026-09-17 에 마친 것: GA4 캠페인 축 추가 · 어드민 유입경로 링크 화면(`/admin/utm`) ·
`/go/` 짧은 주소 · 캠페인별 방문→신청 전환율 표. 전부 운영 반영·확인 완료.
컨플루언스 WYE-89 문서도 같은 날 현재 소스 기준으로 갱신 완료.
...
| 오방 네이버카페 배너 유입 확인 | 2026-09-17 기준 **아직 미게재**라 유입 0건이 정상이다. 게재 후 `naver_cafe_oban / display · main_banner` 로 잡히는지 확인 |

**다시 파지 않아도 되는 것** — `jamfit / listing` 태그(우리가 만든 링크가 아니며 2026-09-17
무시하기로 결정), `jeonbang_app`·`bbabang` 채널(시온님 확인 완료).
