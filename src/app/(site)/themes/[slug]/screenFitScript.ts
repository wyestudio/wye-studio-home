 ...
 *   4. 너무 작아져야 들어가는 블록(후기처럼 원래 긴 것)은 **아예 손대지 않는다**(원래 크기).
 *      한 화면에 못 들어가는 블록이라 줄여봐야 글씨만 작아진다. 평소처럼 스크롤된다.
 *      (2026-09-16: 기본 배율까지는 줄였더니 후기 블록이 작아졌다는 의견)
 *
...
export function fitScreen(section: HTMLElement): void {
  ...
  if (shown + pt + vh * PB_RATIO <= avail) {
    ...
    if (s >= MIN_SCALE) {
      ...
      // 한 화면에 못 들어가는 긴 블록 — 기본 배율도 걸지 않고 원래 크기로 둔다.
      scale = 1;
    }
  }
