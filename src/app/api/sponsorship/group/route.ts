...
export async function POST(request: NextRequest) {
  ...
  if (!Number.isFinite(birthYear) || !Number.isFinite(followers) || !Number.isFinite(reach) || !Number.isFinite(companions)) {
    return NextResponse.json({ error: "숫자 항목을 확인해주세요." }, { status: 400 });
  }

  const supabase = await createClient();
  const { data, error } = await supabase.rpc("submit_sponsorship_group_application", {
    p_name: name,
  ...
  after(async () => {
    try {
      await sendSponsorshipApplicationSlackAlert({ type: "group", name, handle });
    } catch (err) {
