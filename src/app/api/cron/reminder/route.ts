...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // "진행일 하루 전" 에 해당하는 회차를 고른다 (시간 필터라 sessions 에서 먼저).
    //
    // ⚠️ "지금부터 24시간" 이 아니라 **날짜 기준**이다. 시간 기준이면 크론을
    //    하루 한 번(10:00) 돌릴 때 하루 전 안내가 당일 안내로 밀린다:
    //      금 10:00 실행 → 창이 토 10:00 까지 → 토 11:30 회차가 1시간 반 차이로 빠짐
    //      토 10:00 실행에서야 잡혀 "당일" 안내가 된다
    //    환불 규정도 p6 에서 시각(48/24h) → 날짜(4/3/2일) 기준으로 바뀌었으므로
    //    안내 시점도 날짜로 세는 것이 규정과 일관된다.
    //
    // ⚠️ 상한만 "내일 끝" 으로 두고 하한은 now 로 둔다. 날짜가 정확히
    //    일치하는 회차만 고르면, 크론이 하루 걸러 실행되지 못했을 때 그날
    //    대상이던 회차가 **영영 안내를 못 받는다**. 상한만 두면 다음 실행이
    //    놓친 회차까지 같이 잡아 저절로 복구된다(당일 안내가 되긴 하지만
    //    아예 못 받는 것보다 낫다).
    //    이미 보낸 건은 reminder_sms_sent_at 으로 걸러져 중복 발송은 불가능하다.
    const now = new Date();
    const windowEnd = kstDayEnd(now, 1);
