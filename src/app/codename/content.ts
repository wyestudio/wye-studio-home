...
export const ROUNDS: Round[] = [
  ...
  // 1차 마감(2026-09-25~). 2차 날짜가 아직 안 정해져서 **닫아 둔다.**
  //
  // ⚠️ 날짜가 정해지기 전까지 이 줄이 마지막이어야 한다. 예전에는 여기에
  //    "2차 접수 9월 26일 시작" 이 들어 있어서, 날짜만 지나면 아무도 손대지
  //    않아도 2차 접수가 저절로 열리게 돼 있었다. 회차 판정이 날짜 기반이라
  //    1차 마감 다음 날 들어온 제출은 그대로 round = 2 로 저장된다.
  {
    until: "2099-01-01",
    name: "1차 접수 마감",
    dates: "9월 20일~9월 24일 접수 · 발표 9월 25일",
    close: null,
    roundNo: 1,
    closed: true,
  },
...
/*
 * 2차를 열 때는 위 "1차 접수 마감" 줄 **앞에** 아래 두 줄을 되살리고,
 * 날짜(until · dates · close)를 실제 일정으로 고친다.
 *
 *   { until: "2026-10-26", name: "2차 응모 접수 중",
 *     dates: "9월 26일~10월 25일 · 발표 10월 27일",
 *     close: "2026-10-26", roundNo: 2 },
 *   { until: "2099-01-01", name: "비하인드 카드 상시 접수",
 *     dates: "초청권 추첨 종료 · 카드 대상 등록",
 *     close: null, roundNo: 2, always: true },
 *
 * 되살린 뒤 확인할 것:
 *   - 마감 회차(closed: true)가 여전히 배열의 **마지막**인가
 *   - 2차 정답이 1차와 다르면 EVENT_CODENAME_ANSWER 를 먼저 바꿨는가
 *     ← 안 바꾸면 2차 제출이 1차 정답 기준으로 채점된다
 */
...
export const CLOSED = {
  ...
  next: "2차 접수는 일정이 정해지는 대로 오방카페에 공지합니다.",
} as const;
