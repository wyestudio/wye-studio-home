...
const INNER_RADIUS_PX = 70;
const OUTER_RADIUS_PX = 240;
...
const BASE_MIN_RADIUS = 220;
const BASE_MAX_RADIUS = 440;
const RADIUS_TO_MINDIM = 0.42;
const EDGE_MARGIN_PX = 70;
const RADIUS_FLOOR_PX = 110;

type Vec2 = { x: number; y: number };

function computeGeometry(width: number, height: number) {
  const minDim = Math.min(width, height) || 0;
  const desired = Math.min(BASE_MAX_RADIUS, Math.max(BASE_MIN_RADIUS, minDim * RADIUS_TO_MINDIM));
  const edgeSafe = Math.min(desired, width / 2 - EDGE_MARGIN_PX, height / 2 - EDGE_MARGIN_PX);
  const radius = Math.max(RADIUS_FLOOR_PX, edgeSafe);
  const scale = Math.min(1, Math.max(0.55, minDim / 900));
  return { radius, scale };
}

export function MascotOrbit({ progress, reduceMotion }: { progress: number; reduceMotion: boolean }) {
  ...
  useEffect(() => {
    ...
    function measure() {
      ...
      const { radius, scale } = computeGeometry(rect.width, rect.height);
    ...
    function tick(time: number) {
      ...
      // Nearest mascot-to-cursor distance drives ONE shared speed, so the whole
      // orbit slows/resumes together instead of each mascot reacting on its own.
      ...
      const t = clamp01((minDist - INNER_RADIUS_PX) / (OUTER_RADIUS_PX - INNER_RADIUS_PX));
      ...
      MASCOT_ORDER.forEach((id, i) => {
        ...
        if (el) {
          const mascotScale = meta.orbitScale * scale;
          const halfW = (meta.orbitWidth * mascotScale) / 2;
          const halfH = (meta.orbitHeight * mascotScale) / 2;
          el.style.transform = `translate3d(${width / 2 + localX - halfW}px, ${height / 2 + localY - halfH}px, 0) scale(${mascotScale})`;
        }
      });
        ...
        {MASCOT_ORDER.map((id, i) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className="pointer-events-none rounded bg-black/40 px-1.5 py-0.5 text-[10px] text-foreground opacity-0 backdrop-blur-sm transition-opacity duration-150 group-hover:opacity-100 group-focus-visible:opacity-100">
                {meta.name}
              </span>
            </div>
          );
